...
class VAEKNNTrainer(VAETrainer):
    ...
    def _run_epoch(self, step, epoch, dataset, **kwargs):
        ...
        for iteration, ((x_p, label_p), (x_n, label_n)) in enumerate(zip(dlp, cycle(dln))):
            ...
            knn_distance, _ = self.train.get_kneighbors(x.cpu().detach().numpy())
            knn_distance = torch.tensor(knn_distance)

            nll, kld, loss = self.model.loss_function(
                recon_x, x, mean, log_var, y=mask, kl_weight=kl_weight, knn_distance=knn_distance)
