...
class VABCCern(VAE):
    ...
    def loss_function_new(self, recon_x, x, mean, log_var, kl_weight=torch.tensor(1.), recon_log_var=None,
                          gamma_weight=torch.tensor(1.), y=torch.tensor(1.)):

        recon_var = torch.exp(recon_log_var)

        l_vae = dst(recon_x, x, dst_function=self.config.dst_function) / recon_var * 0.5
        ...
        NLL = y * l_vae - (1 - y) * anti_loss(l_vae)
        ...
        return NLL / x.size(0), KLD / x.size(0), loss / x.size(0), (y*l_vae).sum() / x.size(0),((1 - y) * anti_loss(
            l_vae)).sum() / x.size(0)  # , rec, anti_rec
    ...
    def loss_function(self, recon_x, x, mean, log_var, kl_weight=torch.tensor(1.), recon_log_var=None,
        ...
        return NLL / x.size(0), KLD / x.size(0), loss / x.size(0), (y * rec).sum() / x.size(0), -(
                (1 - y) * anti_rec).sum() / x.size(0)
